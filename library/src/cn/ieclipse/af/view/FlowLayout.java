...
public class FlowLayout extends ViewGroup {
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        for (int i = 0; i < count; i++) {
            ...
            if (child != null && child.getVisibility() != GONE) {
                ...
                child.measure(
                        getChildWidthMeasureSpec(child, width,
                                width - x + left),
    ...
    private int getChildWidthMeasureSpec(View child, int width, int rest) {
        LayoutParams lp = (LayoutParams) child.getLayoutParams();
    ...
    private int getChildHeightMeasureSpec(View child, int height) {
        ...
        /* not recommend */
            ...
            if (tmp > 0) {
                childHeightMeasureSpec = MeasureSpec.makeMeasureSpec(tmp,
                        MeasureSpec.AT_MOST);
            }
            else {
            ...
            if (line != null && !line.isEmpty()) {
                if (lineIndex > 0) {
                    if ((mShowVerticalDivider & SHOW_DIVIDER_MIDDLE) != 0) {
                        mVerticalDividerPos
                                .add(new Rect(left, y - getVerticalSpacing(),
                                        getWidth() - right, y));
                    }
                }
                int count = line.size();
    ...
    void drawDividerHorizontal(Canvas canvas) {
        ...
        if (mHasVisibleChild && (getShowDividers() & SHOW_DIVIDER_END) != 0) {
            Drawable d = getDividerDrawable();
            int l = getWidth() - getPaddingRight() - getDividerWidth();
            d.setBounds(l, getPaddingTop() + getDividerPadding(),
                    l + getDividerWidth(),
